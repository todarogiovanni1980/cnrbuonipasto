...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_cnrbuonipasto/assets/css/cnrbuonipasto.css');
$document->addStyleSheet(JUri::root() . 'media/com_cnrbuonipasto/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_cnrbuonipasto');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_cnrbuonipasto&task=consegne.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'consegnaList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
		...
		<div id="j-main-container">
			...
			<table class="table table-striped" id="consegnaList">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canCreate  = $user->authorise('core.create', 'com_cnrbuonipasto');
					$canEdit    = $user->authorise('core.edit', 'com_cnrbuonipasto');
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'consegne.', $canChange, 'cb'); ?>
</td>
						<?php endif; ?>
					...
					<?php echo $item->dipendente; ?>
				</td>				<td>
				<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'consegne.', $canCheckin); ?>
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_cnrbuonipasto&task=consegna.edit&id='.(int) $item->id); ?>">
